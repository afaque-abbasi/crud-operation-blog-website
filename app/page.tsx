...
return (
    <div>
      ...
      {loading ? (
        ...
        <div>
          {/* Check if interpretations is defined and has items */}
            ...
            interpretations.map((interpretation) => (
              <div key={interpretation.$id} className="p-4 my-2 rounded-md leading-9 shadow-sm border-b">
                ...
                <div className="flex justify-end gap-4 mt-4 text-sm">
                  <Link href={`/edit/${interpretation.$id}`}
                  className="bg-slate-200 px-4 py-2 rounded-md uppercase font-bold">
                    Edit
                  </Link>
                  <button onClick={() => handleDelete(interpretation.$id)} className="px-4 py-2 rounded-md uppercase font-bold bg-red-500 text-white">
                    Delete
                  </button>
                </div>
              </div>
